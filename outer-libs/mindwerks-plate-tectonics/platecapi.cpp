...
class platec_api_list_elem {
 public:
  ...
  ~platec_api_list_elem() { delete data; }
};
...
void* platec_api_create(long seed, uint32_t width, uint32_t height,
  ...
  lithosphere* litho = new lithosphere(
      seed, width, height, sea_level, erosion_period, folding_ratio,
...
lithosphere* platec_api_get_lithosphere(uint32_t id) {
  for (uint32_t i = 0; i < lithospheres.size(); ++i)
    if (lithospheres[i].id == id) return lithospheres[i].data;
